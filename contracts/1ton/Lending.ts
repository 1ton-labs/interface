...
import { FUNC_COIN_LENGTH, FUNC_INT_LENGTH, MIN_GAS_FEE } from '@/constants';
...
export class Lending implements Contract {
    ...
    async sendRepay(provider: ContractProvider, via: Sender, item: Address, repayValue: bigint) {
        await provider.internal(via, {
            value: repayValue + MIN_GAS_FEE,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell()
    ...
    async sendDeposit(provider: ContractProvider, via: Sender, depositValue: bigint) {
        await provider.internal(via, {
            value: depositValue + MIN_GAS_FEE,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell()
                .storeUint(Opcodes.DEPOSIT, 32) // op
                .storeUint(0, 64) // query_id
                .storeUint(depositValue, FUNC_COIN_LENGTH)
                .storeUint(depositValue + MIN_GAS_FEE, FUNC_COIN_LENGTH)
                .endCell(),
        })
    }
    ...
    async getLoanPool(provider: ContractProvider) {
        ...
        const result: Map<Address, TonLoan> = new Map();
    ...
    private parseLoan(loanCell: Cell, item: Address): TonLoan {
